...
import { allImages } from './images.js'; // 画像リストをインポート
...
let currentBackgroundImageUrl = allImages && allImages.length > 0 ? allImages[0] : undefined;
...
function setBackground(imageUrl) {
    if (imageUrl) {
        textureLoader.load(
            imageUrl,
            (texture) => {
                texture.mapping = THREE.EquirectangularReflectionMapping; // Or UVMapping depending on image type
...
function initializeImageParticles() {
    ...
    for (let i = 0; i < numFieldParticles; i++) {
        ...
        const imageUrl = allImages[i % allImages.length]; // Cycle through images
